# Markdown 精讲 · 交互式学习网站（Windows 7 单文件版）

> **本副本专为 Windows 7 免环境部署**：`vite.config.js` 已配置 `vite-plugin-singlefile`，
> `npm run build:win7` 会把全站打进一个 `dist/index.html`，双击即可在 Win7 浏览器打开，无需安装任何软件。
> 使用说明见上级目录《使用说明-Windows7.md》。

依据《Markdown精讲交互式网站 - 实施方案（4+1结构）.md》实现的教学网站：
6 大模块、27 课，含双栏实时编辑器、闯关练习、表格构建器、Mermaid 图表工坊、
LaTeX 公式、TOC 生成、AI 辅助写作、表格/SQL 数据分析、文档智能分析、
公文在线转换器等交互功能（模块五全部为本地模拟 AI，零依赖可离线）。
